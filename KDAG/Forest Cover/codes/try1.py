...
W2 = tf.Variable(tf.truncated_normal([L, 7], stddev=0.1, dtype = tf.float64))
B2 = tf.Variable(tf.truncated_normal([7], stddev=0.1, dtype = tf.float64))
#W3 = tf.Variable(tf.truncated_normal([M, N], stddev=0.1, dtype = tf.float64))
#B3 = tf.Variable(tf.truncated_normal([N], stddev=0.1, dtype = tf.float64))
...
for i in range(1000):  
   train_step.run(feed_dict={X: x_train, Y_: y_train, lr: 0.005, pkeep: 0.75})
